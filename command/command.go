...
func ExecuteTransfer(c config.Config, recipient, amount string) ([]byte, error) {
	transferCommand := fmt.Sprintf(faucetExecCommand, c.FaucetWalletAddress, recipient, amount, c.FaucetWalletAlias, c.ChainNode, c.ChainHome, c.KeyringBackend, c.ChainID)
